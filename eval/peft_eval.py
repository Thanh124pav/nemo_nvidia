...
def get_args():
    ...
    parser.add_argument(
        ...
        type = str,
        required = True, 
        help = "path to .txt file to save evaluation"
    )
...
def normalize_answer(s):
    ...
    def handle_specific_tokens(text):
...
def evalSum(args):
    ...
    f1 = exact_match = total = r1 = r2 = r_L = 0
    skip = 0
    for i in range(len(preds)):
        ...
        if pred_answer == "":
            print("Too long document, skip")
            skip += 1
            continue
        true_answer = pred_line[args.label_field]

        r_scores = scorer.score(true_answer, pred_answer)
        r1 += r_scores['rouge1'].fmeasure
        r2 += r_scores['rouge2'].fmeasure
        r_L += r_scores['rougeL'].fmeasure
        exact_match += metric_max_over_ground_truths(exact_match_score, pred_answer, true_answer)
        f1 += metric_max_over_ground_truths(f1_score, pred_answer, true_answer)
        total += 1
    print(f"Number documents to be skipped: {skip}")
    exact_match = 100.0 * exact_match / total
    f1 = 100.0 * f1 / total
    r1 = 100 * (r1 / total)
    r2 = 100 * (r2 / total)
    r_L = 100 * (r_L / total)
    res = {'exact_match': exact_match, 'f1': f1, "rouge1": r1, "rouge2": r2, "rougeL": r_L , 'total': total}
