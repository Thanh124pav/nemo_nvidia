...
MODEL_CONFIG_MAPPING = {
    ...
    "meta-llama/Meta-Llama-3.2-1B": (llm.LlamaModel, llm.Llama32Config1B),
    "meta-llama/Meta-Llama-3.2-3B": (llm.LlamaModel, llm.Llama32Config3B),
    "meta-llama/Meta-Llama-3.2-3B-Instruct": (llm.LlamaModel, llm.Llama32Config3B),
    ...
    if not input_path.is_dir():  # if .nemo tar
        with tempfile.TemporaryDirectory() as tmp_dir:  # we want to clean up this tmp dir
            ...
            if tokenizer_model:
                ...
                print(f"load offline tokenizer from {args.tokenizer_path}")
                HFAutoTokenizer.from_pretrained(args.tokenizer_path, local_files_only = True).save_pretrained(tokenizer_tmp_dir)
