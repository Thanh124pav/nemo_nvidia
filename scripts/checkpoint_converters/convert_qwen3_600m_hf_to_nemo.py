...
Conversion script to convert Huggingface QWen2(QWen1.5) checkpoints into nemo checkpoint.
  Example to run this conversion script:
    python convert_qwen3_600m_hf_to_nemo.py \
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), '../../examples/nlp/language_modeling/conf/megatron_qwen3_600m_config.yaml'
        ),
        required=False,
...
def load_config(args, qwen_config):
    ...
    nemo_config.head_dim = qwen_config['head_dim']
...
def convert(args):
    ...
    head_size = hf_config['head_dim'] # hidden_size // head_num
    ...
    for l in range(int(num_layers)):
        ...
        print(f"q_proj.shape: {model.state_dict()[f'model.layers.{l}.self_attn.q_proj.weight'].shape}")
        print(f"new_q_tensor_shape: {new_q_tensor_shape}")
